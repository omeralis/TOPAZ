...
export class CaseListComponent implements OnInit {
  ...
  openEdit(data: any) {
    ...
    this.myForm.addControl(
      'case_description',
      this.fb.control(data.case_description)
    );
  ...
  FromDynamically() {
    this.http.get('/assets/form-config.json').subscribe((formData: any) => {
  ...
  onSubmit() {
    ...
    if (this.myForm.value.id) {
      this.caseService
        .UpdateCase(this.myForm.value.id, this.myForm.value)
        .subscribe(
          (res) => {
            this.isLoading = false;
